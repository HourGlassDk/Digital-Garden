...
const extractBaseSlug = (filePath: string): string => {
...
const getDataForBacklinks = (fileNames: string[], filePath: string) => {
  const allPosts = fileNames
    .map((fileName) => {
      ...
      let version = frontmatterVersion as number | undefined;
      if (!version) {
    ...
    .filter((post): post is NonNullable<typeof post> => post !== null); // Remove null entries (drafts)
...
(function () {
  ...
  const posts: PostWithLinks[] = totalPostData.map(
