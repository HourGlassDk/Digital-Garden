...
const bracketsExtractor = (str: string): string[] => {
  if (!str) return [];
  const matcher = /((?!\])(?!\[).)+/gs;
 ...
 */
const stripExcerpt = (str: string): string =>
  str
...
const getExcerpt = (str: string): string => {
  if (!str) return "";
...
const getDataForBacklinks = (fileNames: string[], filePath: string): PostData[] =>
...
(function () {
  ...
  const posts: LinkData[] = totalPostData.map(
